-- ================================================================
--  CLUB SAVINGS LEDGER — FULL SCHEMA V2
--  Supports: Member shares (multiples of 500), flat fines, 
--            FIFO allocation, and club settings.
-- ================================================================
...
DECLARE
  ...
  v_due_day      SMALLINT;
BEGIN
  SELECT savings_start_year, savings_start_month, base_share_value, late_fine_amount, due_day
  INTO   v_start_year, v_start_month, v_base_deposit, v_fine, v_due_day
...
BEGIN
  ...
  SELECT savings_start_year, savings_start_month, base_share_value, late_fine_amount, due_day
  INTO   v_start_year, v_start_month, v_base_deposit, v_fine_amt, v_due_day
  FROM   club_settings WHERE id = 1;
  ...
  LOOP
    ...
    WHILE v_remaining >= v_required LOOP
      SELECT year, month INTO v_future_year, v_future_month FROM monthly_ledger WHERE member_id = p_member_id AND status = 'advance' ORDER BY year DESC, month DESC LIMIT 1;
      IF NOT FOUND THEN
...
-- (Omitted complex views for brevity, can be added as needed)
